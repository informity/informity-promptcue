...
class TestDecisionEngine:
    ...
    def test_no_candidates_confidence_band_low(
        self,
        decision_engine: PromptCueDecisionEngine,
    ) -> None:
        result = decision_engine.resolve(PromptCueClassificationResult(candidates=[]))
...
class TestClassifierTiers:
    def test_trigger_match_fires_for_known_phrase(
        self,
        det_classifier: PromptCueClassifier,
    ) -> None:
...
class TestInjectableEmbedFn:
    ...
    def test_encode_empty_returns_empty_without_calling_fn(self) -> None:
        ...
        def counting_fn(_text: str) -> list[float]:
            nonlocal call_count
            call_count += 1
...
class TestTemporalScope:
    ...
    def test_year_range_fires(self) -> None:
        ...
        assert (
            _detect_requires_multi_period_analysis("Compare performance from 2020 to 2023") is True
        )
    ...
    def test_last_n_years_fires(self) -> None:
        ...
        assert (
            _detect_requires_multi_period_analysis("Over the last 3 years, what changed?") is True
        )
    ...
    def test_quarterly_trend_fires(self) -> None:
        ...
        assert (
            _detect_requires_multi_period_analysis("Show the quarterly trend for incidents") is True
        )
    ...
    def test_comparison_no_year_no_fire(self) -> None:
        ...
        assert _detect_requires_multi_period_analysis(query) is False
