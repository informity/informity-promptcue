...
def test_linguistic_extracts_entities(linguistic_analyzer: PromptCueAnalyzer) -> None:
    # Aurora, OpenSearch, and AWS are recognisable entities in en_core_web_sm.
    result = linguistic_analyzer.analyze(_QUERY)
    assert len(result.entities) > 0, "Expected at least one named entity"
...
keybert_mod = pytest.importorskip(
    "keybert",
    reason="KeyBERT not installed — skipping keyword tests",
)
