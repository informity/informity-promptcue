...
def test_entities_include_surface_text_and_type() -> None:
    analyzer = PromptCueAnalyzer(PromptCueConfig(enable_linguistic_extraction=True))
    result = analyzer.analyze("Compare Aurora and OpenSearch for RAG workloads on AWS")
    assert all(entity.text for entity in result.entities)
    assert all(entity.entity_type for entity in result.entities)
